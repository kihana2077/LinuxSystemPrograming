        ...
        for(int i = 0;i<=max_fds;i++){
            if(FD_ISSET(i,&readfds)){
                if(i == server_fd){
                    ...
                    if(temp_fd == -1){
                        continue;
                    }else if(temp_fd >= MAX_CONNECTIONS){
                        close(temp_fd);
                        continue;
                    }
                    client[temp_fd].client_fd = temp_fd;
                    client[temp_fd].used = 0;

                    printf("One client has connect.\n");
                    if(client[temp_fd].client_fd > max_fds){
                        max_fds = client[temp_fd].client_fd;
                    }
                    FD_SET(client[temp_fd].client_fd,&master_read);
                    continue;
                }else{
